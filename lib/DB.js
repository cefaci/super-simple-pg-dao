...
export default class DB {
  ...
  static timeEnd(name, options, start){
    ...
    console.log('\tDB->query():', '\x1b[32m'+options.name, timeMS, 'ms', out)
  }
   ...
   * @param {*} tables 
   */
  static async task(query, options, values, tables) {
    options = options || {}

    const start = process.hrtime.bigint()
    return await db.task(options.name, async(t) => {
      return query(t, options, values)
    }).then(data => {
      DB.timeEnd('\tDB->query():', options, start)
      return tables ? Preparation.result_object(data, tables): data // Promise.resolve(tables ? Preparation.result_object(data, tables): data)
    }).catch(error => {
