...
export default class DB {
  ...
  static update(t, options, values) {
    let qs = Sql.update(options, values)
    console.debug('SQL:', qs)
    return t.one(qs, options.values)
  }

  static delete(t, options, values) {
    let qs = Sql.delete(options, values)
    // console.debug('SQL:', qs)
    return t.one(qs, options.values)
  }
  ...
  static async query(query, options, values, tables) {
    ...
    return await query(db, options, values)
      .then(data => {
        ...
        // console.debug('\tDB->query():', options, values)
  ...
  static async queryTx(query, options) {
    ...
    return await db.tx(async(t) => {
      ...
      console.log('\tDB->queryTx():', options)
  ...
  static async queryTxBatch(options) {
    ...
    return await db.tx(async(t) => {
      ...
      for(let i = 0; i < options.length; i++){
        ...
        let query = typeof sql === 'undefined' ? options[i] : Sql[sql](options[i], options[i].values)
      ...
      console.log('\tDB->queryTxBatch():', options)
  ...
  static async queryTxBatchStepWithReturnFK(options) {
    ...
    return await db.tx(async(t) => {
      ...
      console.log('\tDB->queryTxBatchStepWithReturnFK():', options)
